...
import { ArrowUpDown, BarChart2, Info, Settings, Users, LogOut, CalendarDays } from 'lucide-react';
...
const HorarioEditable = () => {
  ...
  const planHoy = pasoActual ? planificacion[pasoActual.id]?.[fechaHoyISO] : null;
  ...
  const cargarGuardia = (guardiaAUsar = guardiaElegida, turnosPreset = {}, horariosEspecialesPreset = {}) => {
    ...
    const candidatos = agentesIdentidad.filter(
      (a) => a.paso === pasoActual.id && (guardias.length === 0 || a.guardia === guardiaAUsar)
    );
    const porAgente = {};
  ...
  <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
    <div className="bg-white p-4 rounded w-96">
      ...
      <div className="mb-4 flex space-x-4 items-center flex-wrap gap-2">
        ...
        <Link to="/planificacion" className="text-gray-500 hover:text-gray-700 flex items-center text-sm">
          <CalendarDays size={16} className="mr-1" /> Planificación
        </Link>
      ...
      {operativoPaso.activosHoyIds.length === 0 ? (
        <div className="bg-white p-4 rounded shadow mb-4">
          ...
          {planHoy && planHoy.guardia ? (
            <div>
              <p className="text-sm mb-2">
                Guardia planificada para hoy: <strong>{planHoy.guardia}</strong> (turnos ya cargados desde{' '}
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => cargarGuardia(guardiaElegida)} disabled={!guardiaElegida} className="bg-blue-500 text-white p-2 rounded disabled:opacity-50">
                Cargar guardia
              </button>
              <span className="text-xs text-gray-400">
                (sin planificación para hoy — <Link to="/planificacion" className="underline">cargarla</Link>)
              </span>
            </div>
          ) : (
